...
void kmain(uint64_t totalMemory, uint64_t freeMemory, uint64_t lastAddress, EFI_GRAPHICS_OUTPUT_PROTOCOL *framebuffer) {
    ...
    while (1) {
        ...
        if (KAOS_DISPLAY_WALLPAPER) {
            drawPicture(picture_wallpaper, bitmap, 0, 0, picture_wallpaper_width, picture_wallpaper_height,
                        ALPHA_COLOR);
        } else {
        ...
        drawFillRectangle(bitmap, terminalX + terminalRow * KAOS_FONTS_WIDTH + 8 - KAOS_FONTS_WIDTH,
                          terminalY + KAOS_FONTS_HEIGHT * terminalLine, 2, KAOS_FONTS_HEIGHT, 0x00dddddd);
        ...
        drawPicture(picture_cpu, bitmap, hInfoX + 10, hInfoY + KAOS_GUI_WINDOW_TITLE_HEIGHT + 10, picture_cpu_width,
                    picture_cpu_height, ALPHA_COLOR);
        drawPicture(picture_ram, bitmap, hInfoX + 10, hInfoY + KAOS_GUI_WINDOW_TITLE_HEIGHT + 20 + picture_cpu_height,
                    picture_ram_width, picture_ram_height, ALPHA_COLOR);
        drawString(bitmap, hInfoX + 10 + picture_cpu_width + 5,
                   hInfoY + KAOS_GUI_WINDOW_TITLE_HEIGHT + 10 + KAOS_FONTS_HEIGHT / 2, 0x00000000, cpuInfo());
        drawString(bitmap, hInfoX + 10 + picture_ram_width + 5,
                   hInfoY + KAOS_GUI_WINDOW_TITLE_HEIGHT + 20 + picture_cpu_height + KAOS_FONTS_HEIGHT / 2, 0x00000000,
                   memInfo());
